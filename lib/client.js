...
(function() {
  ...
  Client = (function() {
    ...
    */
    ...
    function Client(cacheClient) {
      ...
      QUERY_CACHE(this.cacheObj, this.opts.queryFunctions);
    }
    ...
    */
    ...
    Client.prototype.addMongodbConfig = function(config) {
      ...
      if (_.isArray(config)) {
        ...
        if (immediatelyInit) {
          return this._init(dbName, config.uri, config.options, config.auth);
        }
      }
    };
    ...
    */
    ...
    Client.prototype.handle = function() {
      var args, cacheObj, cbf, collectionName, dbName, handleName, key, options, queryFunctions, se, self, tasks, ttl, wrapCbf, _i;
      ...
      wrapCbf = function(err, data) {
        ...
        if (data && _.isFunction(data.toArray)) {
          ...
          toArrayArgs._se = se;
          return self.tasks.add(self._toArray, toArrayArgs, self);
        } else {
      ...
      self.collection(dbName, collectionName, function(err, collectionObj) {
        ...
        if (err) {
          ...
          if (handleName !== 'find') {
            ...
            return self.tasks.add(collectionObj[handleName], args, mergeSameHandle, collectionObj);
          } else {
            return collectionObj[handleName].apply(collectionObj, args);
          }
        }
      });
      return self;
    ...
    */
    ...
    Client.prototype._init = function(dbName, uri, options) {
      ...
      _.each(uris, function(uri) {
        var db, _base, _ref, _ref1;
        uri = url.parse(uri);
        if (uri.path) {
          db = uri.path.substring(1);
        }
        if (db) {
          if ((_ref = connectionInfos[db]) == null) {
            connectionInfos[db] = {};
          }
          if ((_ref1 = (_base = connectionInfos[db]).servers) == null) {
            _base.servers = [];
          }
          connectionInfos[db].auth = uri.auth;
          return connectionInfos[db].servers.push({
            host: uri.hostname,
      ...
      _.each(connectionInfos, function(value, name) {
        var auth, authInfos, db, replicaSetOptions, server, serverOptions, serverOptionsKey;
        ...
        if (!self.dbInfo(dbName)) {
          ...
          db.open(function(err, db) {
            var completeCbf, _ref;
            if (err) {
              return logger.error("init db " + dbName + " fail, msg:" + err);
            } else {
              completeCbf = function() {
                logger.info("init db " + dbName + " success!");
                self.db(dbName, db);
                return self.dbInfo(dbName, 'complete');
              };
              if (auth) {
                if ((_ref = auth.type) == null) {
                  auth.type = 'admin';
                }
                return db[auth.type](function(err, authenticateDb) {
                  return authenticateDb.authenticate(auth.name, auth.pwd, function(err, result) {
                    if (err) {
                      return logger.error('authenticate is fail, #{err}');
                    } else {
                      return completeCbf();
                    }
                  });
    ...
    */
    ...
    Client.prototype._toArray = function(cursor, cbf) {
      cursor.toArray(cbf);
      return this;
    };
  ...
  */
  ...
  QUERY_CACHE = function(cacheObj, cacheFunctions) {
  ...
  */
  ...
  VALIDATE = function() {
    ...
    Client.prototype.handle = _.wrap(Client.prototype.handle, function() {
      var args, cbf, checkData, data, func, handleName, schema, self;
      func = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      self = this;
      handleName = args[2];
